function [units] = spiketimeidentificationklustakwik(X,nEig,upsample, sr,...
    thrFactor, show)
% [units] = spiketimeidentificationklustakwik(X,nEig,upsample,sr,...
%                                             thrFactor,show)
%
...
for i=1:dims;
    ...
    N_SAMPLES = length(x);
    if(show);figure(fig1);subplot(splt_size,splt_size,i);
        plot(x);hold on;
    end
    ...
    if ~isempty(amplitudes)
        ...
        f_tot = pre + post + 1;
        %take only those peaks for which the desired window is entirely
        %contained in the data:
        amplitudes = amplitudes( ((indices - pre) >= 1) & ...
            ((indices + post) <= N_SAMPLES) );
        indices = indices( ((indices - pre) >= 1) & ...
            ((indices + post) <= N_SAMPLES) );
        ...
        if N_CLU > 1
            ...
            units(i).separability = abs(meanAmpWinner)/noise_std;
            %corresponds to average signal to noise ratio (noise is assumed
            %to have unit variance) there is no nearest neighbour or 
            %background
            units(i).IsoINN = NaN;
            units(i).IsoIBg = NaN;
        end
        ...
        if show
            ...
            title(strcat('amplSD = ',num2str(units(i).amplitudeSD,2),...
                         '; RSTD = ',num2str(units(i).RSTD,2),...
                         '; sep. = ',num2str(units(i).separability,2),...
